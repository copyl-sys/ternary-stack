 ...
 * Enhanced with FFT multiplication for T81BigInt.
 */
...
#define TS_PRINT(fmt, ...) do { time_t t = time(NULL); printf("[%s %s] " fmt, log_names[LOG_INFO], ctime(&t), ##__VA_ARGS__); } while (0)
#endif
...
/* ============================================================
 * Section 2: Axion Kernel Module (Unchanged)
 */
#ifdef __KERNEL__
...
static void fft(double *a, size_t n, int inverse) {
 ...
 double t_im = odd[k] * w_re + odd[k] * w_im;
 a[k] = even[k] + t_re;
 a[k + n / 2] = even[k] - t_re;
 if (inverse) {
 a[k] /= 2;
 ...
 *result = TS_MALLOC(sizeof(T81BigInt));
 ...
 if (carry) {
 TS_PRINT("Warning: Carry overflow in FFT multiplication\n");
 }
...
int discrete_opcode_execute(const char *encoded_opcode, int operand1, int operand2) {
 ...
 case TEXP: {
 const int MOD = 9973;
 int res = 1, base = operand1 % MOD, exp = operand2;
 while (exp > 0) {
...
static void skip_spaces(void) {
 while (*g_expr && isspace(*g_expr)) g_expr++;
}

static T81BigInt *parse_number(void) {
 skip_spaces();
 int sign = TERNARY_POSITIVE;
 if (*g_expr == '-') {
 sign = TERNARY_NEGATIVE;
 g_expr++;
 }
 skip_spaces();
 if (!(*g_expr == '1' || *g_expr == '0' || *g_expr == 'T')) {
 fprintf(stderr, "Error: Expected a balanced ternary digit at: %s\n", g_expr);
 return NULL;
 }
 size_t len = 0;
 const char *start = g_expr;
 while (*g_expr == '1' || *g_expr == '0' || *g_expr == 'T') {
 len++;
 g_expr++;
 }
 T81BigInt *num = TS_MALLOC(sizeof(T81BigInt));
 if (!num || allocate_t81bigint(num, len) != TERNARY_NO_ERROR) {
 TS_FREE(num);
 ...
 num->sign = sign;
 for (size_t i = 0; i < len; i++) {
 char digit = start[len - 1 - i];
 num->digits[i] = (digit == '1') ? 1 : (digit == 'T') ? -1 : 0;
 }
 return num;
}

static T81BigInt *parse_factor(void);
static T81BigInt *parse_term(void);
...
static T81BigInt *parse_factor(void) {
 ...
 skip_spaces();
 if (*g_expr != ')') {
 fprintf(stderr, "Error: Expected ')' at: %s\n", g_expr);
...
static T81BigInt *parse_term(void) {
 T81BigInt *result = parse_factor();
 if (!result) return NULL;
 skip_spaces();
 while (*g_expr == '*' || *g_expr == '/') {
 char op = *g_expr;
 g_expr++;
 T81BigInt *rhs = parse_factor();
 if (!rhs) {
 free_t81bigint(result);
 TS_FREE(result);
 return NULL;
 ...
 if (op == '*') {
 TernaryError err = t81bigint_mul(temp, rhs, &result);
 ...
 TernaryError err = t81bigint_div(temp, rhs, &result);
 if (err != TERNARY_NO_ERROR) {
 fprintf(stderr, "Error: Division failed%s\n", err == TERNARY_ERR_DIVZERO ? " (division by zero)" : "");
 free_t81bigint(temp);
 TS_FREE(temp);
 free_t81bigint(rhs);
 TS_FREE(rhs);
...
static T81BigInt *parse_expression(void) {
 T81BigInt *result = parse_term();
 if (!result) return NULL;
 skip_spaces();
 while (*g_expr == '+' || *g_expr == '-') {
 char op = *g_expr;
 g_expr++;
 T81BigInt *rhs = parse_term();
 if (!rhs) {
 free_t81bigint(result);
 TS_FREE(result);
 return NULL;
 ...
 T81BigInt *temp = result;
 if (op == '+') {
 TernaryError err = t81bigint_add(temp, rhs, &result);
 if (err != TERNARY_NO_ERROR) {
 ...
 rhs->sign = (rhs->sign == TERNARY_POSITIVE) ? TERNARY_NEGATIVE : TERNARY_POSITIVE;
 TernaryError err = t81bigint_add(temp, rhs, &result);
 if (err != TERNARY_NO_ERROR) {
...
T81BigInt *tritjs_eval_expression(const char *expr) {
 if (!expr) {
 T81BigInt *zero = TS_MALLOC(sizeof(T81BigInt));
 if (allocate_t81bigint(zero, 1) != TERNARY_NO_ERROR) {
 TS_FREE(zero);
 return NULL;
 }
 zero->sign = TERNARY_ZERO;
 zero->digits[0] = 0;
 return zero;
 }
 g_expr = expr;
 T81BigInt *result = parse_expression();
 skip_spaces();
 if (*g_expr != '\0') {
 fprintf(stderr, "Error: Unexpected input at: %s\n", g_expr);
 free_t81bigint(result);
 TS_FREE(result);
 return NULL;
 }
...
void print_help(void) {
 printf("Ternary System Utility Program\n");
 printf("=============================================================\n");
 printf("Available options:\n");
 printf(" expr \"expression\" : Evaluate a ternary arithmetic expression (e.g., \"1T+0\", \"11/1T\").\n");
 printf(" hanoi n : Solve Tower of Hanoi with n disks.\n");
 printf(" ser filename : Serialize a sample 3x3 matrix.\n");
 printf(" des filename : Deserialize and perform matrix ops.\n");
 printf(" opcode number : Encode and validate an opcode.\n");
 printf(" execopcode <enc> <op1> <op2> : Execute opcode on operands.\n");
...
void instruction_parser(void) {
 char *line;
 printf("Ternary System Interactive Mode. Type \"help\" for instructions.\n");
 while (1) {
 line = readline("ts> ");
 if (!line) break;
 ...
 T81Matrix *m = TS_MALLOC(sizeof(T81Matrix));
 fread(m, sizeof(T81Matrix), 1, f);
 m->data = TS_MALLOC(m->rows * m->cols * sizeof(T81BigInt));
 fread(m->data, sizeof(T81BigInt), m->rows * m->cols, f);
 fclose(f);
 ...
 printf("Encoded opcode %s is %s.\n", encoded, discrete_opcode_validate(encoded) ? "valid" : "invalid");
 }
 } else if (strcmp(token, "execopcode") == 0) {
 char *enc = strtok(NULL, " \t\n");
 char *op1_str = strtok(NULL, " \t\n");
 char *op2_str = strtok(NULL, " \t\n");
 if (!enc || !op1_str || !op2_str || !isdigit(*op1_str) || !isdigit(*op2_str)) {
 ...
 printf("Result of executing opcode %s on %d and %d: %s\n", enc, op1, op2, buffer);
