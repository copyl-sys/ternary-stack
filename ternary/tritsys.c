...
static const char *usage_text =
    ...
    "  -n, --hanoi N         Solve Tower of Hanoi for N disks\n"
    ...
    "  tritsys -n 3            # Prints Hanoi moves\n"
    ...
    while (1) {
        ...
        if (strcmp(token, "help") == 0) {
            ...
            if (!n_str || !isdigit(*n_str)) {
                fprintf(stderr, "tritsys: hanoi requires a number\n");
            } else {
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "e:n:s:d:o:x:ih", long_options, NULL)) != -1) {
        switch (opt) {
            case 'e': {
            ...
            case 'n': {
                if (!isdigit(*optarg)) {
                    fprintf(stderr, "tritsys: hanoi requires a numeric argument\n");
                    return 1;
                }
                solve_hanoi(atoi(optarg));
                return 0;
            }
            case 's': {
